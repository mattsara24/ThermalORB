...
void Viewer::Run()
{
    ...
    if(mpTracker->mSensor == mpSystem->MONOCULAR || mpTracker->mSensor == mpSystem->STEREO || mpTracker->mSensor == mpSystem->RGBD || mpTracker->mSensor == mpSystem->THERMAL_MONOCULAR)
    {
        menuShowGraph = true;
    }
